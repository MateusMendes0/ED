...
void Fila::Retirar(string remover){
	
	Fila aux;
	Dado auxDado;
	
	while(!Vazia()){
		auxDado = Desenfileirar();
		if(auxDado.assunto != remover){
			aux.Enfileirar(auxDado);
			}
		}
	
	while(!aux.Vazia()){
		auxDado = aux.Desenfileirar();
		Enfileirar(auxDado);
		}
	}
